...
comp3 <- comp2 %>% mutate(GC_avg = rowMeans(select(comp2, c(D7K40d1_191011_NextSeq, D7K40d2_191011_NextSeq))), leaf_avg = rowMeans(select(comp2, c(D6L40d1_190903_NextSeq_R1, D6L40d2_190903_NextSeq_R1, D6L40d3_190919_NextSeq))))
```
...
ATG_GC <- comp4 %>% filter(more_AS == "GC") %>% 
  select(GeneID)
write.table(ATG_GC, "C:/Users/hasna/Documents/MSc_project/vasttools/all_data/dPSI25_no_merging_all_reps/K40d-L40d_168events/GC_up_ATGs.txt", sep = "\n", row.names = F, na = "", quote = F, col.names = F)
ATG_leaf <- comp4 %>% filter(more_AS == "leaf") %>% 
  select(GeneID)
write.table(ATG_leaf, "C:/Users/hasna/Documents/MSc_project/vasttools/all_data/dPSI25_no_merging_all_reps/K40d-L40d_168events/leaf_up_ATGs.txt", sep = "\n", row.names = F, na = "", quote = F, col.names = F)
```
...
```{r}
GO_output <- read_delim( "C:/Users/hasna/Documents/MSc_project/vasttools/all_data/dPSI25_no_merging_all_reps/K40d-L40d_168events/leaf_up_GO.txt", delim = "\t")
...
wide$entries_002 <- str_remove(wide$entries_002, "/")

#okkk looks good. now pivot longer to get one row per GO-AGI pair
long <- wide %>% pivot_longer(entries_002:entries_100, names_to = "waste_of_space", values_to = "AGI_code") %>% 
...
```{r}
#all GO terms
tiff("test.tiff", units="in", width=10, height=15, res=300)
