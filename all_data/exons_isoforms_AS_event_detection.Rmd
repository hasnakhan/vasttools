...
this is idea number 5 in the figure wishlist. The idea here is to probe the notion that AS event detection can be increased using a de novo assembly method. To get at this, we're going to find out if the genes with detected AS events (defined in Cmero et al 2021 as 10 <= PSI <= 90) have a higher than expected number of isoforms in the database. To do this, we need to:
```{r}
library(tidyverse)
```
...
gtf$GeneID <- str_split(gtf$X9, ";", simplify = T)[,1]
gtf$transcript <- str_split(gtf$X9, ";", simplify = T)[,2]
gtf$GeneID <- str_sub(gtf$GeneID, 10, 18)
gtf$transcript <- str_sub(gtf$transcript, 17, 27)
